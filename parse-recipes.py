...
bakingIngredients = ['ammonia', 'baking', 'eggs',
                     'flour', 'margarine', 'yeast', 'bisquick®']
cookingFats = ['butter', 'gelatin', 'gravy', 'lard',
               'lecithin', 'ovalette', 'shortening', 'xanthan', 'suet']
...
descriptionsWithPredecessor = ['removed', 'discarded', 'reserved',
...
for recipeId in range(6660, 27000):
    ...
    if soup:
        ...
        parsedTitle = title.lower().replace("(", "").replace(")",
                                                             "").replace("-", " ").split(" ")
        ...
        # 2 spans with "Add all" and 1 empty
        ...
        for i in range(0, count):
            ...
            for i in range(0, len(parsedIngredient)):
                pluralUnit = inCheckingPlurals(
                    parsedIngredient[i], measurementUnits)
                if pluralUnit:
                    ...
                    if i < len(parsedIngredient) and parsedIngredient[i] == "+":
                        while "+" in parsedIngredient:
                            ...
                            ingredient["amount"] += transformToCups(
                                eval(parsedIngredient[index]), parsedIngredient[index+1])
                            del parsedIngredient[index]
            ...
            if parsedIngredient[0] == "or":
                pluralUnit = inCheckingPlurals(
                    parsedIngredient[1], measurementUnits)
                if pluralUnit:
            ...
            while index < len(parsedIngredient):
                ...
                # word not in descriptions, check if description with predecessor
                    ...
                    descriptionString = parsedIngredient[index -
                                                         1] + " " + word
                    del parsedIngredient[index - 1]
            ...
            if ingredientString == "":
                outputFile.write("Bad ingredient string: {0}".format(
                    ingredientObjects[i].text))
                ingredientString = ingredientObjects[i].text
            ...
            ingredient["labels"] = getLabelsFromArray(
